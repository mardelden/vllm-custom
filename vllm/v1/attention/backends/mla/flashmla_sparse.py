...
class FlashMLASparseBackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        if cache_dtype_str in ("fp8_ds_mla", "fp8"):
            # V3.2 main MLA: 656-byte custom storage format. See module docstring.
            return (num_blocks, block_size, 656)
        # plans/101 dev18498 skew: gpu_model_runner._reshape_kv_cache_tensors gates
        # the forwarded cache_dtype_str on kv_quant_mode, which is NONE for
        # fp8_ds_mla (an MLA storage format, not a per-token-head quant mode), so it
        # passes "auto" here even though MLAAttentionSpec.real_page_size_bytes has
        # already sized the allocation as block_size*656. This sparse backend serves
        # the 576 DS-MLA head_size (512 NoPE + 64 RoPE) ONLY via that 656 packed
        # layout, so return it to match the allocation. (A bf16 run also has
        # head_size 576 but a 1152B page → mismatches loudly on the numel check,
        # never silent corruption.)
        if cache_dtype_str == "auto" and head_size == 576:
            return (num_blocks, block_size, 656)
