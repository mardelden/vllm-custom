...
| `codex/qwen3-vl-skip-zero-dummy-modalities` | `768c007dc6` | Preliminary v5 RTX/Qwen multimodal experiment: skip normal video dummy setup when video count is zero | **Rebased and pushed 2026-08-31 as `codex/qwen3-vl-skip-zero-video` (`13dd9c9858`)**, where it still applies to current upstream. Still unmeasured; carried for durability, not on a benchmark. See `plans/005`. |
...
| `codex/startup-plan-stable-fingerprint` | `2c49d626cd` | Isolated v5 RTX startup-plan correctness experiment | **Ref deleted 2026-08-31.** It was local-only and fully redundant: its `_get_worker_plan_fingerprint` is byte-identical to the copy in `codex/fastsafetensors-parallel-mtp-share`, which is on origin and also carries the schema-2 and exact-KV-reuse work built on top of it. Nothing was lost. |
